                ...
                parallel(
                  ...
                  Ches: {
                    try {
                      ...
                      echo 'Ches Backend build failed'
                      notifyStageStatus('ChesBackend', 'FAILURE')
                      throw e
                    }
                  },
            ...
            openshift.withCluster() {
              openshift.withProject(TOOLS_PROJECT) {
                if(DEBUG_OUTPUT.equalsIgnoreCase('true')) {
                  ...
                  def bcChesBackendConfig = openshift.selector('bc', "${APP_NAME}-${JOB_NAME}-ches-backend")
                  ...
                  if(bcChesBackendConfig.exists()) {
                    echo "Removing BuildConfig ${APP_NAME}-${JOB_NAME}-ches-backend..."
                    bcChesBackendConfig.delete()
                  }
...
def deployStage(String stageEnv, String projectEnv, String hostEnv, String pathEnv) {
  ...
  openshift.withCluster() {
    openshift.withProject(projectEnv) {
      ...
      if(!(openshift.selector('cm', 'ches-config').exists() &&
      ...
      parallel(
        ...
        Ches: {
          ...
          createDeploymentStatus(projectEnv, 'PENDING', hostEnv, pathEnv)
          def dcChesBackend = openshift.apply(dcChesBackendTemplate).narrow('dc')
          timeout (time: 10, unit: 'MINUTES') {
            try {
              def cbmf = dcChesBackend.rollout()
              cbmf.latest()
              cbmf.status()
            } catch (e) {
              // deployments are unreliable to track when rolling and multiple pods...
              // infrequently throw an error that deploy is already happening.
              // when you process a deployconfig without triggers, openshift will still create a Config change trigger, so timing is everything
              echo "Ches Backend error on rollout: ${e}"
            }
          }
        },
