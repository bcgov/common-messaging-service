...
def deployStage(String stageEnv, String projectEnv, String hostRouteEnv) {
  ...
  openshift.withCluster() {
    openshift.withProject(projectEnv) {
      ...
      def latestVersion = openshift.selector('dc', "${APP_NAME}-frontend-${JOB_NAME}").object().status.latestVersion
      def rc = openshift.selector('rc', "${APP_NAME}-frontend-${JOB_NAME}-${latestVersion}")
      timeout(5) {
        rc.watch {
